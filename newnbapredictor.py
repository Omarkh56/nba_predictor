...
def get_todays_games() -> List[Dict]:
    for days_ahead in range(-1, 45):  # include preseason / early-season slate
        ...
        try:
            resp = requests.get(
                f"{ESPN_SCOREBOARD_URL}?dates={date_str}", headers=ESPN_HEADERS, timeout=12
            )
...
def get_espn_injuries() -> Dict[str, List[Dict]]:
    ...
    for team_entry in data.get("injuries", []):
        ...
        team_name = team_entry.get("displayName", "") or team_entry.get("team", {}).get(
            "displayName", ""
        )
        if not team_name:
            continue
...
def _fetch_all_team_data(season: str) -> pd.DataFrame:
    ...
    # ─── Merge everything ────────────────────────────────────────────────
    df = (
        adv.merge(base, on="TEAM_ID", how="left")
...
def _fetch_last_n(
    team_id: int, season: str, all_ratings: pd.DataFrame, n: int = FORM_GAMES
) -> dict:
    ...
    ).head(max(n, 15))  # grab 15 to detect road trips
...
def compute_four_factors_edge(home_row, away_row, league_avgs: dict) -> Tuple[float, dict]:
    ...
    factors["TOV%"] = {
        "home_off": h_tov_off,
        "home_forces": h_tov_def,
        "away_off": a_tov_off,
        "away_forces": a_tov_def,
    ...
    factors["OREB%"] = {"home_off": h_oreb, "away_off": a_oreb, "edge": round(oreb_home_edge, 4)}
    ...
    factors["FTA_RATE"] = {"home_off": h_fta, "away_off": a_fta, "edge": round(fta_home_edge, 4)}
...
def _estimate_total(home_row, away_row, league_avg_off):
    ...
    h100 = (
        league_avg_off
        + (home_row.get("OFF_RATING", 112) - league_avg_off)
        + (away_row.get("DEF_RATING", 112) - league_def)
    )
    a100 = (
        league_avg_off
        + (away_row.get("OFF_RATING", 112) - league_avg_off)
...
def _compute_prediction(
    m: dict, ratings: pd.DataFrame, season: str, injuries: dict, league_avgs: dict
) -> Optional[Dict]:
    ...
    # Factor 2: Injuries
    h_inj_adj, h_inj_det = compute_injury_adj(
        m["home_name"], injuries, float(home_row.get("PPG", 112))
    )
    a_inj_adj, a_inj_det = compute_injury_adj(
        m["away_name"], injuries, float(away_row.get("PPG", 112))
    )
    ...
    return {
        ...
        "away_l10": away_form["w_pct"],
        "home_l10": home_form["w_pct"],
...
def predict_games_data(
    ...
    league_avgs = {
        ...
        "FG3_PCT": float(ratings["FG3_PCT_OFF"].mean())
        if "FG3_PCT_OFF" in ratings.columns
        else 0.36,
...
def _print_results(results: List[Dict], season: str):
    ...
    print("  Factors: NetRtg · Injuries · FourFactors · Matchups · Rest")
    print("           Form · HCA · Pace · Clutch · H2H")
    print(f"{'═' * w}")
    ...
    for r in results:
        ...
        print(
            f"  │  Score:     {r['away_abbr']} {r['away_score']:.0f}  -  "
            f"{r['home_abbr']} {r['home_score']:.0f}              │"
        )
        print(f"  │  Favorite:  {r['favorite']} ({r['fav_prob']:.1%}){'':20}│")
        print("  └───────────────────────────────────────────────┘")
        ...
        print(
            f"    ③ Four Factors:          {b['four_factors']:>+6.2f} pts  "
        ...
        if r["h_inj_det"] or r["a_inj_det"]:
            ...
            if r["h_inj_det"]:
                ...
                for d in r["h_inj_det"][:5]:
                    ...
                    print(f"      {d['player']:<22} {d['status']:<13} {d['adj']:>+5.1f} pts{ppg_s}")
            if r["a_inj_det"]:
    ...
    # ── Summary table ────────────────────────────────────────────────────
    print(f"\n{'═' * w}")
    print("  PREDICTED WINNERS — FULL SLATE SUMMARY")
    print(f"  {'─' * 70}")
    print(
        f"  {'#':<3}  {'MATCHUP':<14}  {'PICK':<5}  {'WIN%':>6}  {'SPREAD':>8}  {'O/U':>6}  CONFIDENCE"
    )
    print(f"  {'─' * 70}")
