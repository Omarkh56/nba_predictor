...
def _load_today_lean(snap_date: date) -> dict:
    ...
    try:
        ...
        if lean_map:
            print(f"  Book lean loaded: {len(lean_map)} player-market lines for {snap_date.isoformat()}")
    except Exception as e:
...
def _rank_score(edge_pct: float, confidence: float, market: str,
                pick: str = "OVER", line: float = 99.0) -> float:
...
def _match_odds_event(all_events, home_abbr, away_abbr):
    ...
    for ev in all_events:
        if _abbr(ev.get("home_team","")) == home_norm and \
           _abbr(ev.get("away_team","")) == away_norm:
            return ev
    return None
...
def _edge_floored(df: pd.DataFrame) -> pd.DataFrame:
    ...
    book_ok = (df["book_count"] >= 3) if "book_count" in df.columns else pd.Series(True, index=df.index)
    edge_ok = df.apply(
        lambda r: r["Edge%"] >= props_model.UNDER_EDGE_PCT_FLOOR if r["Pick"] == "UNDER"
...
# =============================================================================
_PROP_HDR = (f"  {'Player':<22} {'Mkt':<8} {'Line':>5} {'Proj':>5}"
             f"   {'E%':>5}   {'Conf':>6}   {'Pick':<5}")
_PROP_SEP = (f"  {'─'*22} {'─'*8} {'─'*5} {'─'*5}"
             f"   {'─'*5}   {'─'*6}   {'─'*5}")
...
def _pick_detail_line(r) -> str:
    ...
    show_prefixes = ("DvP", "TREND", "★STRONG", "HI-USG", "LO-USG",
                     "BLW-", "USG+", "NO-PO", "LOW-PO")
    key_flags = [f for f in str(r.get("Flags", "")).split()
                 if any(f.startswith(p) for p in show_prefixes)]
    if key_flags:
...
def print_team_section(pred: dict):
    ...
    spread_str = (f"{ha} {pred['home_spread']:+.1f}" if margin > 0
                  else f"{aa} {pred['away_spread']:+.1f}")
    conf_str   = ("Strong ✓" if pred["fav_prob"] >= 0.68
                  else "Lean" if pred["fav_prob"] >= 0.57 else "Toss-up")
    pace_str   = _pace_label(hp, ap)
    IW = WIDTH - 4   # inner width

    print(f"  ┌{'─'*IW}┐")
    # Line 1: matchup, spread, O/U, pace
    l1 = (f"{aa} {pred['away_prob']:.1%}  vs  {ha} {pred['home_prob']:.1%}"
          f"   Sprd: {spread_str}   O/U: {pred['predicted_total']:.1f}   {pace_str}")
    print(f"  │  {l1:<{IW-3}}│")
    # Line 2: score, fav, confidence
    l2 = (f"Proj: {aa} {pred['away_score']:.0f} – {ha} {pred['home_score']:.0f}"
          f"   Fav: {fav} {pred['fav_prob']:.1%}   [{conf_str}]")
    print(f"  │  {l2:<{IW-3}}│")
    # Factor lines
    f1 = (f"NetRtg {b['net_rtg']:+.1f}  Inj {b['injuries']:+.1f}  "
          f"FF {b['four_factors']:+.1f}  Form {b['form']:+.1f}  Rest {b['rest']:+.1f}")
    f2 = (f"HCA {b['hca']:+.1f}  Pace {b['pace']:+.1f}  "
    ...
    sig_inj = [d for d in pred.get("h_inj_det", []) + pred.get("a_inj_det", [])
...
def _devil_advocate_risks(r) -> list:
    ...
    if entry.get("near_miss_rate", 0) >= 0.30:
        risks.append(f"Misses within 1 unit {entry['near_miss_rate']:.0%} of the time — historically thin edge")
    if "NO-PO" in flags:
...
def _print_pick_row(r, rank: int = 0, show_rank: bool = False, show_verbose: bool = False):
    ...
    print(f"  {rank_col}{r['Player']:<22} {team_col}{r['Market']:<8} "
    ...
    if show_verbose:
        ...
        for sc, d in dets.items():
            ...
            print(f"    └ {sc}: {d.get('rate',0):.4f}/min"
        ...
        if bp  > 0:    pen.append(f"bench={bp:.1f}")
        if vp  > 0:    pen.append(f"vol={vp:.1f}")
        if dp  > 0:    pen.append(f"dir_pen={dp:.1f}")
        if uf != 1.0:  pen.append(f"usg={uf:.3f}")
        if pen: print(f"    └ penalties: {', '.join(pen)}")
...
def print_props_tables(results: list) -> pd.DataFrame:
    ...
    if notes:
        print(f"  ⚠  {' · '.join(notes)}")
    print(f"  {len(df)} props analyzed   "
          f"★ = High ≥66%   * = below breakeven 53%")
    return df
...
def print_best_bets(pred: dict, props_df: pd.DataFrame):
    ...
    for marker, r, signal_str, be_tag in rows:
        ...
        line1 = (f"  {marker} {r['Player']:<22} {r['Market']:<8} {r['Pick']:<5} "
...
def _pick_to_dict(row: pd.Series, game_key: str, pred) -> dict:
    ...
    return {
        ...
        "game_pace":   float((pred.get("home_pace", 99.5) +
                               pred.get("away_pace", 99.5)) / 2) if pred else None,
...
def _init_calibration() -> None:
    ...
    if _CALIB_RANK_WEIGHTS:
        ...
        print(f"  Calib weights loaded ({len(_CALIB_RANK_WEIGHTS)} markets) — "
