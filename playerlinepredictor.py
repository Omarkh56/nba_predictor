...
import dvp as dvp_engine  # Defense vs Position module  # noqa: E402
...
MIN_MINUTES_RS = 15
MIN_MINUTES_PO = (
    10  # lowered from 18 — rotation players with 10-17 min PO appearances were being excluded
)
...
PLAYOFF_NO_PO_PENALTY = (
    8.0  # extra penalty when IS_PLAYOFF and po_count==0 (RS rates ≠ playoff role)
)
...
PROP_STD_DEV_PO = {
    ...
    "player_assists": 2.6,
    "player_threes": 1.6,
    "player_points_rebounds": 7.8,
    "player_points_assists": 7.6,
    "player_rebounds_assists": 4.3,
    "player_points_rebounds_assists": 8.2,
}
PROP_STD_DEV_RS = {
    "player_points": 6.5,
    "player_rebounds": 3.0,
    "player_assists": 2.8,
    "player_threes": 1.5,
    "player_points_rebounds": 7.2,
    "player_points_assists": 7.1,
    "player_rebounds_assists": 4.1,
    "player_points_rebounds_assists": 7.8,
}
...
_LEARNED_PARAMS_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "learned_params.json"
)
...
def _load_learned_player_params() -> None:
    ...
    try:
        ...
        print(
            f"  [train_model] player params loaded  "
                ...
                for split in block.get("splits", []):
                    for cat in split.get("categories", []):
                        for s in cat.get("stats", []):
                            ...
                            if nm in ("avgpoints", "ppg"):
                                ppg = v
                            elif nm in ("avgrebounds", "rpg"):
                                rpg = v
                            elif nm in ("avgassists", "apg"):
                                apg = v
            injs.append(
                {
...
def prefetch_rosters_for_games(events):
    ...
    if seen:
        ...
        if _manual_added:
            print(
                f"  Manual positions applied for {len(_manual_added)} player(s): "
                f"{', '.join(sorted(_manual_added))}"
            )
        print(
            f"  DvP roster positions loaded for {len(seen)} team(s) "
...
def get_all_odds_events():
    ...
    r = requests.get(
        f"{BASE_URL}/sports/{SPORT}/events", params={"apiKey": ODDS_API_KEY}, timeout=20
    )
    r.raise_for_status()
    events = r.json()
...
def consensus_lines(props_df):
    ...
    lines_df = (
        over_df.groupby(["player", "market"])["line"]
        .median()
        .reset_index()
    ...
    book_counts = (
        over_df.groupby(["player", "market"])["bookmaker"]
        .nunique()
        .reset_index()
...
def _fetch_logs(
    ...
    # min_minutes in cache key prevents po_logs (min=10) / po_raw (min=0) collision.
    ...
    result = safe_dataframe_call(
        _fetch,
        retries=retries,
        label=f"fetch_logs pid={pid} {season_type}",
    )
    PLAYER_LOGS_CACHE[ck] = result
    return result
...
# LEAGUE LOGS
# =============================================================================
def safe_lgf(retries: int = 3, **kw) -> pd.DataFrame:
    def _call():
        df = leaguegamefinder.LeagueGameFinder(timeout=30, **kw).get_data_frames()[0]
        api_sleep()
        return df

    return safe_dataframe_call(_call, retries=retries, label="safe_lgf")
...
def get_league_logs():
    ...
    df = safe_lgf(
        season_nullable=SEASON, season_type_nullable=SEASON_TYPE_PO, player_or_team_abbreviation="P"
    )
    if df.empty:
        print("  Playoff logs empty — falling back to regular season.")
        df = safe_lgf(
            season_nullable=SEASON,
            season_type_nullable=SEASON_TYPE_RS,
...
def get_team_stat_avg(team_abbr, stat_col):
    ...
    if tid is None or tid not in ts.index:
        return 0.0
...
def get_def_factor(opp_team_id, stat_col):
    ...
    if ck in TEAM_DEF_CACHE:
        return TEAM_DEF_CACHE[ck]
    logs = get_league_logs()
    if logs.empty or stat_col not in logs.columns:
        TEAM_DEF_CACHE[ck] = 1.0
        return 1.0
...
def get_series_def_factor(opp_team_id, stat_col, h2h_logs):
    ...
    series_facing = logs[(logs["GAME_ID"].isin(series_game_ids)) & (logs["TEAM_ID"] != opp_team_id)]
...
# =============================================================================
def _get_injured_player_recent_stat(injured_name: str, inj_field: str, n_games: int = 10):
...
def compute_usage_boost(
    player_name, player_pos, stat_col, player_avg, team_avg, injuries_for_team, using_playoffs=False
):
    ...
    for inj in injuries_for_team:
        ...
        player_claim = player_share_of_team * (
            SAME_POS_SHARE / 0.20 if same_pos else DIFF_POS_SHARE / 0.80
        )
...
def _per_min_rate(logs, stat_col, max_g=20):
    ...
    if sub.empty or stat_col not in sub.columns:
        return None, None
    if "MIN" not in sub.columns:
        return None, None
...
def get_series_h2h(logs, opp_abbr, stat_col, base_rate, max_games=7):
    ...
    h = (
        logs[logs["MATCHUP"].str.contains(rf"\b{opp_abbr}\b", na=False, regex=True)]
        .head(max_games)
        .copy()
    )
    if len(h) < 1:
        return 1.0
    if "MIN" not in h.columns:
        return 1.0
...
def _ha_factor(logs, is_home, stat_col):
    if logs.empty or "MATCHUP" not in logs.columns or stat_col not in logs.columns:
        return 1.0
    mask = (
        logs["MATCHUP"].str.contains(r"\bvs\.", na=False)
        if is_home
        else logs["MATCHUP"].str.contains(r"@", na=False)
    )
    vg = logs[mask]
...
def _is_road_b2b(po_logs, is_home):
    ...
    if not (is_away_0 and is_away_1):
        return False
...
def project_stat(
    ...
    if abs(spread) > BLOWOUT_SPREAD_THRESHOLD and avg_min >= STARTER_MIN_THRESHOLD:
        ...
        if player_is_favored:
            mins_reduced = min(
                BLOWOUT_MAX_MIN_REDUCTION, (abs(spread) - BLOWOUT_SPREAD_THRESHOLD) * 0.5
            )
    ...
    usage_boost = compute_usage_boost(
        player_name, pos, stat_col, player_avg, team_avg, injuries, using_playoffs=using_playoffs
    )
...
def _self_injury_check(player_name: str, injuries: list):
    ...
    status = next((inj["status"] for inj in injuries if inj.get("player") == player_name), None)
...
def _compute_prop_range(market_key: str, po_logs, rs_logs, po_count):
    ...
    proj_low, proj_high = (
        _projection_range(range_logs, primary_stat) if primary_stat else (None, None)
    )
...
def _apply_combo_and_usg(
    total: float, details: dict, stat_cols: list, market_key: str, player_name: str
):
...
def project_prop(
    ...
    calib_proj_adj = (
        CALIB.get("player_market", {}).get(f"{player_name}|{calib_label}", {}).get("adj", 0.0)
    )
    ...
    for logs in [po_logs, rs_logs]:
        ...
        for _, r in recent.iterrows():
            ...
            if t == h:
                ...
                ac += 1
        if hc > ac:
            return {"team_abbr": home_abbr, "opp_abbr": away_abbr, "is_home": True}
        elif ac > hc:
            return {"team_abbr": away_abbr, "opp_abbr": home_abbr, "is_home": False}
    return None
...
def _build_player_context(
    cdf, home_name, away_name, home_abbr, away_abbr, home_tid, away_tid, injuries_by_team
) -> dict:
...
def _build_prop_flags(
    ...
    dominant_trend = (
        "declining"
        if trend_labels.count("declining") > len(trend_labels) // 2
        else "improving"
        if trend_labels.count("improving") > len(trend_labels) // 2
        else "neutral"
    )
    ...
    is_strong = (pick_dir == "OVER" and proj_low is not None and proj_low > line) or (
...
def process_game(
    ...
    if not bookmakers:
        ...
        print("  Props typically go up 2–4 hours before tip-off. Try again later.")
        return []

    print(
        f"  {len(bookmakers)} book(s), {sum(len(b.get('markets', [])) for b in bookmakers)} market(s)"
    )
    ...
    for _, row in cdf.iterrows():
        ...
        flags = _build_prop_flags(
            ev, meta, details, book_flag, pick_dir, proj, line, po_count_local, po_raw_count_local
        )
...
def _prefetch_spreads(gday):
    ...
    for ev in gday:
        ...
        if game_key not in GAME_SPREADS:
            ...
            if sprd != 0.0:
                print(
                    f"    {game_key}: spread {sprd:+.1f} (home favored)"
                    if sprd > 0
